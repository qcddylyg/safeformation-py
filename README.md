...
**基于屏障约束与 PD/ADP 控制的多智能体安全编队仿真**

**技术/方法：**Python、MATLAB、NumPy、RK4 数值积分、PD 控制、Actor-Critic ADP、启发式屏障约束、非线性动力学、通信延迟与输入饱和。

- 针对多智能体编队中的障碍物规避、通信保持及外部扰动问题，构建 Leader-Follower 安全编队场景，综合考虑通信距离、障碍物距离、输入饱和与非线性动力学。
- 将论文中的 MATLAB 场景迁移为 Python/RK4 仿真工具链，设置“有无屏障 × PD/ADP”四组对比：低增益 PD、启发式屏障-PD、普通 ADP（无屏障）和启发式屏障-ADP，并测试动态障碍物、50/100 ms 通信延迟、质量增加 20% 和扰动增强等工况，对比编队误差、安全距离、通信约束、控制能量和饱和率。
- 统一实验结果表明：低增益 PD 与普通 ADP 在名义场景发生障碍物越界；启发式屏障-ADP 在当前测试场景中保持无越界，100 ms 延迟下越界次数为 0，而启发式屏障-PD 为 121 次。该结果是特定参数和本地障碍物测量假设下的数值观察，不代表普适安全定理。
