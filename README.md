...
- 将论文中的 MATLAB 场景迁移为 Python/RK4 仿真工具链，设置“有无屏障 × PD/ADP”四组对比：低增益 PD、启发式屏障-PD、普通 ADP（无屏障）和启发式屏障-ADP，并测试动态障碍物、50/100 ms 通信延迟、质量增加 20% 和扰动增强等工况。
- 统一比较编队误差、安全距离、通信违例、控制能量和饱和率：低增益 PD 与普通 ADP 在名义场景发生障碍物越界；屏障-ADP 在当前测试场景中保持无越界，且在 100 ms 延迟下为 0 次越界，而屏障-PD 为 121 次。该结果是特定配置下的数值观察，不代表普适安全定理。
